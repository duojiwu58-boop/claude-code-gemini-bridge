...
为了让 Gemini 3.8 Flash 的极速推理、大上下文和超强性能在 Claude Code 中 **100% 完全发挥**，我用 Rust 和 Delphi 写了这个深度协议适配的多模型智能体运行时并开源了：

👉 **GitHub 仓库：** https://github.com/duojiwu58-boop/claude-code-multi-model-agent-runtime
📦 **Release v0.8.0 下载：** https://github.com/duojiwu58-boop/claude-code-multi-model-agent-runtime/releases/tag/v0.8.0
...
6. 🖱️ **Gemini 3.8 Flash 原生 Computer Use**：Gemini 决策，Claude Code 自动托管 stdio MCP，本机 Host 执行 Browser/Desktop 截图与输入；无需打开模型中心，不调用第二个模型。
7. 🖥️ **Delphi 11 VCL 原生 GUI + Windows 服务**：极轻量原生 Windows GUI 界面，支持不重启 Claude Code 秒切 Gemini / DeepSeek / Kimi / Opus 等不同模型路由；支持一键安装为 Windows 系统服务。
...
**Title:** [Open Source] Claude Code Multi-Model Agent Runtime with deep Gemini 3.8 Flash compatibility (Windows Service & Native GUI)

**Body:**

Hi everyone!

Claude Code's terminal agent interface is fantastic, but using Google Gemini 3.8 Flash as its backend through generic API proxies usually falls apart due to missing thinking streams, schema validation errors on MCP tools, or broken `thought_signature` state in multi-turn tool loops.
...
📦 **Release v0.8.0:** https://github.com/duojiwu58-boop/claude-code-multi-model-agent-runtime/releases/tag/v0.8.0
...
- 🖱️ **Native Gemini 3.8 Flash Computer Use**: Gemini plans actions, Claude Code automatically manages a local stdio MCP lifecycle, and a signed-in-user Windows Host executes Browser/Desktop screenshots and input without a second model.
...
🚀 **v0.8.0 major upgrade** for Claude Code Multi-Model Agent Runtime!

A protocol-aware Rust Agent Runtime that makes @GoogleAI Gemini 3.8 Flash run as a first-class backend inside @AnthropicAI Claude Code!

✨ Real-time Thinking Stream
🔑 Thought Signature Loop
🛠️ MCP JSON Schema Cleaning
📄 PDF & Vision Tool Results
🖱️ Gemini 3.8 Flash Computer Use via Claude-managed stdio MCP
...
**标题：** 让 Gemini 3.8 Flash 成为 Claude Code 的超级大脑：多模型智能体运行时开源！

**摘要：** v0.8.0 针对 Gemini 3.8 Flash 新提供的原生 Computer Use 能力完成重大升级：Gemini 决策、Claude Code 自动托管本地 stdio MCP、Windows Host 执行 Browser/Desktop 操作，无需打开模型中心或调用第二个模型；同时保留思考流、Tool Call 签名、多模态、Schema 清洗和安全边界。提供一键 Setup 安装包与免安装 ZIP。
