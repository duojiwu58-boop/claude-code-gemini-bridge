...
- Major upgrade for the native Computer Use capability newly provided by Gemini 3.8 Flash. Gemini remains the only decision-making model; the bridge aligns Gemini Interactions with Claude tool contracts, Claude Code owns local tool scheduling and lifecycle, and a signed-in-user Windows Host performs screenshots and input without a manually opened Model Center or second model.
- Added a Windows x64 Gemini Computer Use Browser executor for Claude Code: `computer_start`, ordered idempotent batches, `computer_cancel`, native Gemini action/result translation for unary and streaming Interactions, restart-safe original call identity, an isolated localhost-only 1440×900 browser, per-action PNGs, bounded safety approvals, packaging, and deployment support.
- Added phase-two Windows desktop Computer Use: explicit GUI HWND selection, Windows Graphics Capture screenshots, 17 SendInput actions, embedded PerMonitorV2/asInvoker manifest, multi-display physical-coordinate mapping, selected-window/child/owned-dialog scoping, elevated and sensitive-window blocking, default-desktop checks, and a two-second target lifecycle watchdog.
- Replaced the manually launched, service-polled Computer Host with an installer-registered local `gemini-computer` stdio MCP Server. Claude Code now owns the Host lifecycle directly; window selection and one-shot safety confirmation are the only Host UI, and the bridge service no longer exposes Host registration, heartbeat, polling, result, or Computer management routes.
- Fixed the explicit Computer Use exit path: active turns that request `computer_cancel` expose and force only the exact MCP cancel function, avoiding Gemini's rejection of `computer_use` combined with search/function providers. A successful cancel now closes the current lifecycle exactly once, while a later `computer_start` in the same Claude Code conversation opens a fresh context.
- Added request-level Anthropic MCP connector interoperability on Gemini Interactions: `mcp_servers` and matching `mcp_toolset` declarations now become validated native Remote MCP servers, bearer credentials become redacted headers, Gemini-safe name normalization and allowlists are retained, and unsupported denylists fail closed. Unary and streaming Gemini MCP steps are reconstructed as Claude `mcp_tool_use`/`mcp_tool_result` blocks. Claude Bash, Text Editor, and Memory client declarations expand into schema-defined Gemini functions, tool-search declarations use eager function exposure, and `search_result` inputs remain available as attributed text context.
- Mapped current Anthropic versioned `web_search_*`, `web_fetch_*`, and `code_execution_*` declarations to Gemini Interactions native tools with profile-aware deduplication and explicit diagnostics for controls Gemini cannot enforce. Added `gemini_service_tier: "auto"`, Claude fast-mode to Gemini Priority mapping, actual `usage.speed`/`usage.service_tier` reporting, and Flex-aware 20-minute request/stream-idle windows. Schema-less Claude Computer/Browser toolsets now produce an actionable MCP/client-executor diagnostic instead of disappearing silently.
- Deep-adapted the native Gemini path to GA `gemini-3.8-flash`: runtime, installer, examples, Claude settings, smoke scripts, and current documentation now use the new model by default while retaining 3.7 compatibility. The bridge enforces the documented `low`/`medium`/`high` Thinking range, rejects assistant prefill for current and future Flash generations, and explicitly reports removal of unsupported sampling, candidate-count, frequency-penalty, and presence-penalty fields. The existing current-schema Interactions implementation continues to preserve `steps`, SSE deltas, thought signatures, strict function-result identity, structured output, and stored continuation.
- Updated Tokio, Axum, Reqwest, Tower HTTP, URL, the tracing stack, and Windows bindings to current stable compatible lines, moving the HTTP client TLS stack from rustls 0.21 to 0.23 and clearing the locked `idna` and tracing-subscriber RustSec advisories. Native Anthropic passthrough now applies a two-minute per-read idle timeout, which resets after each received body chunk without imposing a whole-stream deadline.
...
- Preserved Anthropic user-profile/OpenRouter attribution request headers plus upstream rate-limit and tracing response headers. `/v1/models` now identifies the active profile model and reports both Claude 5 models' official 1M context and 128k output limits instead of a stale bridge default. OpenRouter token counting remains explicitly estimated because its current Anthropic API exposes no native count endpoint. The locked Rust suite now contains 225 bridge tests plus 3 Computer Host tests, all passing.
...
### Gemini 3.8 Flash 原生 Computer Use

- 针对 Gemini 3.8 Flash 新提供的原生 Computer Use 能力完成重大升级。Gemini 始终是唯一负责画面理解与动作决策的模型；桥接器负责 Gemini Interactions 与 Claude 工具协议对齐，Claude Code 负责本地工具调度和生命周期，Windows 用户态 Host 负责截图与输入，不需要手动打开模型中心，也不调用第二个模型。
- 新增 Windows x64 Browser 执行器：提供 `computer_start`、顺序幂等批次和 `computer_cancel`，支持普通与流式 Interactions 的原生 Gemini 动作/结果转换、可跨重启重建的原始调用身份、仅 localhost 的隔离 1440×900 浏览器、逐动作 PNG、安全确认、打包和部署。
- 新增第二阶段 Windows Desktop Computer Use：真实用户显式选择 HWND，使用 Windows Graphics Capture 截图和 SendInput 的 17 个动作；内嵌 PerMonitorV2/asInvoker manifest，支持多屏物理坐标，限制在所选窗口、子窗口与同进程关联对话框范围，并阻止提权、敏感窗口和非 Default 输入桌面。
- 用安装器注册的本地 `gemini-computer` stdio MCP Server 取代 GUI 手动启动、服务轮询式 Host。Claude Code 自动管理 Host 生命周期；只有选窗和一次性安全确认会显示 GUI，桥接服务不再承担 Host 注册、心跳、轮询、结果或 Computer 管理接口。
- 修复显式退出生命周期：活动上下文收到 `computer_cancel` 请求时只暴露并强制精确的 MCP 取消函数，避免 Gemini 拒绝 `computer_use` 与搜索/函数工具混用。成功取消只执行一次并立即结束当前上下文，同一 Claude Code 对话之后仍可通过新的 `computer_start` 开启新会话。
- 新增 Gemini Interactions 的 Claude 请求级 MCP connector 互操作：`mcp_servers` 与匹配的 `mcp_toolset` 会转换为经过校验的原生 Remote MCP，Bearer 凭据转为脱敏 header，保留 Gemini 安全名称规范化与 allowlist，无法执行的 denylist 则 fail closed。普通与流式 Gemini MCP steps 会还原为 Claude `mcp_tool_use`/`mcp_tool_result` 块。Claude Bash、Text Editor、Memory 客户端声明会展开成 Gemini 可调用的带 schema 函数，tool-search 声明改为立即暴露函数，`search_result` 输入则保留为带来源的文本上下文。
- 将 Claude 当前版本化 `web_search_*`、`web_fetch_*`、`code_execution_*` 声明映射为 Gemini Interactions 原生工具，并按 profile 去重；Gemini 无法强制执行的控制项会产生明确诊断。新增 `gemini_service_tier: "auto"`、Claude fast mode 到 Gemini Priority 的映射、实际 `usage.speed`/`usage.service_tier` 回报，以及 Flex 感知的 20 分钟请求/流空闲窗口。Claude schema-less Computer/Browser toolset 不再静默消失，而会提示使用 MCP/客户端执行器路径。
- 将原生 Gemini 路径深度升级到 GA `gemini-3.8-flash`：运行时、安装器、示例、Claude settings、冒烟脚本和当前文档均以新模型为默认，同时保留 3.7 兼容。桥接器强制使用官方规定的 `low`/`medium`/`high` Thinking 范围，对当前及未来 Flash 版本拒绝 assistant prefill，并明确报告已移除的不支持采样参数、候选数、频率惩罚和存在惩罚字段。现有新版 Interactions 实现继续完整保留 `steps`、SSE delta、思维签名、严格函数结果身份、结构化输出和 stored continuation。
- Tokio、Axum、Reqwest、Tower HTTP、URL、tracing 栈和 Windows 绑定已升级到当前稳定兼容版本，HTTP 客户端 TLS 栈从 rustls 0.21 迁移到 0.23，并清除了锁文件中的 `idna` 与 tracing-subscriber RustSec 公告。原生 Anthropic 直通现在采用两分钟的逐次读取空闲超时，每次收到响应字节后重新计时，不会引入整流总时限。
...
- 保留 Anthropic 用户 profile/OpenRouter 归属请求头及上游限流、追踪响应头。`/v1/models` 现在使用活动 profile 的真实模型 ID，并报告两个 Claude 5 模型官方 1M 上下文与 128k 输出上限，不再显示过期的桥接默认模型。由于 OpenRouter 当前 Anthropic API 没有原生计数端点，Token Count 仍明确标记为估算。锁定 Rust 测试现为 225 项桥接器测试加 3 项 Computer Host 测试，全部通过。
